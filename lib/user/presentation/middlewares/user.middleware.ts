...
export class UsersMiddleware implements IUsersMiddleware {
    ...
    async validateUserExists(req: express.Request, res: express.Response, next: express.NextFunction) {
        const user = await this.userService.readById(req.params.userId);
        if (user) {
            next();
        } else {
...
//export default IUsersMiddleware ;

export default interface IUsersMiddleware {
    validateUserExists: (req: express.Request, res: express.Response, next: express.NextFunction) => Promise<any>;
}